...
const filters = ['Broad reading', 'Most influential', 'Recent work', 'Open access', 'Dataset-backed', 'Citation paths']
...
export default function DiscoveryPage() {
  ...
  const [activeFilters, setActiveFilters] = useState<string[]>(['Recent work', 'Open access'])
  ...
  const papers = [...((data || []) as Paper[])]
    .filter((p) => {
      if (!q.trim() || activeFilters.includes('Broad reading')) return true
    ...
    .sort((a, b) => {
      if (activeFilters.includes('Most influential')) return b.citation_count - a.citation_count
      if (activeFilters.includes('Recent work')) return b.published_at.localeCompare(a.published_at)
      return 0
    })
  ...
  return (
    <div className="space-y-5">
      <PageHeader eyebrow="Discover" title="Research Landscape" description="Move through papers, citation paths, datasets, and adjacent ideas as an evolving field of evidence." />
      <section className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-5">
        <div className="mb-5">
          <p className="font-serif text-3xl">What field are you entering?</p>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-500">
            Start broad, then follow similar papers, citation paths, and dataset-backed work into a sharper research direction.
          </p>
        </div>
        ...
        <div className="mt-4 flex flex-col gap-2 sm:flex-row">
          <input
            ...
            placeholder="Enter a research question, field, author, dataset, or method..."
          />
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-3">
          <div className="flex items-center justify-between px-1">
            <p className="font-mono text-xs uppercase tracking-[0.2em] text-zinc-500">
              {papers.length} papers shaping this landscape
            </p>
            <button
          ...
          {papers.map((p) => (
            <article key={p.id} className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-4 transition-colors hover:border-zinc-600">
              <div className="mb-1 flex flex-wrap items-center gap-2">
                ...
                {activeFilters.includes('Dataset-backed') && ['paper-008', 'paper-014', 'paper-016', 'paper-018'].includes(p.id) && (
                  <span className="rounded bg-green-950/40 px-2 py-0.5 font-mono text-[10px] text-green-400">dataset linked</span>
                )}
                {activeFilters.includes('Citation paths') && (
        ...
        <aside className="space-y-4">
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5">
            <h2 className="font-serif text-2xl">Landscape Notes</h2>
            {[
              ...
              ...(activeFilters.includes('Dataset-backed') ? [`Dataset-backed leads: ${papers.filter((p) => ['paper-008', 'paper-014', 'paper-016', 'paper-018'].includes(p.id)).length}`] : []),
            ].map((item) => (
          ...
          {activeFilters.includes('Citation paths') && !citationTrace && (
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5">
              <h2 className="font-serif text-2xl">Citation Paths</h2>
              <p className="mt-3 text-sm leading-6 text-zinc-400">
                Citation path mode is active. Use “Trace citations” on any paper to inspect how ideas travel through the field.
              </p>
              <button onClick={() => navigate('/graph')} className="mt-4 w-full rounded-lg border border-zinc-700 px-3 py-2 text-sm text-zinc-300">
                Open full graph
              </button>
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5">
            <h2 className="font-serif text-2xl">Next Moves</h2>
            {[
              ...
              ['Reason with Copilot', () => { localStorage.setItem('axiom-copilot-draft', `Analyze this research landscape for ${q || 'latest AI research'}.`); navigate('/copilot') }],
            ].map(([label, action]) => (
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5">
            <h2 className="font-serif text-2xl">Adjacent Fields</h2>
